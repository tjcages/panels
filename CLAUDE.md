# Panels — agent instructions

Floating React control panel for any live tool (`@tjcages/panels`). Compiles out of prod builds. Read `README.md` first. Previously `shader-dev` / `shader-panel`; the package and repo are now `panels`.

## Linear tracking (non-negotiable)

Every agent, every session. Linear: **team "Off-brand"**, **project "Panels"**.
